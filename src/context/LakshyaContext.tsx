...
interface LakshyaContextType {
  ...
  // Goal actions
  addGoal: (name: string, type: GoalType, deadline: string, classTracking: string, levelNames?: string[], classEnabled?: boolean, startDate?: string, idealDate?: string) => void;
...
export const LakshyaProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const recalculateGoalCompletion = (children: GoalChild[]): number => {
    if (children.length === 0) return 0;
    
    // Help calculate a single node's percentage recursively
    const getNodeCompletion = (node: GoalChild): number => {
      if (!node.children || node.children.length === 0) {
        return node.completed ? 100 : 0;
      }
      const sum = node.children.reduce((acc, c) => acc + getNodeCompletion(c), 0);
      return Math.round(sum / node.children.length);
    };

    const total = children.reduce((acc, c) => acc + getNodeCompletion(c), 0);
    ...
    setGoals(prev =>
      prev.map(g => {
        if (g.id === goalId) {
          ...
          const completion = recalculateGoalCompletion(alignedChildren);
          return {
            ...g,
  ...
  const bulkAddChildren = (goalId: string, parsedRows: string[][]) => {
    ...
    parsedRows.forEach((row) => {
      ...
      if (levelCount === 1) {
        // Simple 1-tier list
        const name = cols[0];
        const exists = tree.some(n => n.name.toLowerCase() === name.toLowerCase());
        if (!exists) {
          tree.push({
            id: generateId(),
            name,
        ...
        // 2-tier tree (e.g. Subject -> Chapter)
        const lvl1Name = cols[0];
        const lvl2Name = cols[1] || '';

        let lvl1Node = tree.find(n => n.name.toLowerCase() === lvl1Name.toLowerCase());
        if (!lvl1Node) {
          lvl1Node = {
            id: generateId(),
            name: lvl1Name,
        ...
        if (lvl2Name) {
          ...
          const lvl2Exists = lvl1Node.children.some(n => n.name.toLowerCase() === lvl2Name.toLowerCase());
          if (!lvl2Exists) {
            lvl1Node.children.push({
              id: generateId(),
              name: lvl2Name,
              completed: false,
              children: [],
        ...
        // 3-tier tree (e.g. Subject -> Chapter -> Topic)
        const lvl1Name = cols[0];
        const lvl2Name = cols[1] || '';
        const lvl3Name = cols[2] || '';
        ...
        if (lvl2Name) {
          if (!lvl1Node.children) lvl1Node.children = [];
          let lvl2Node = lvl1Node.children.find(n => n.name.toLowerCase() === lvl2Name.toLowerCase());
          if (!lvl2Node) {
            lvl2Node = {
              id: generateId(),
              name: lvl2Name,
          ...
          if (lvl3Name) {
            ...
            const lvl3Exists = lvl2Node.children.some(n => n.name.toLowerCase() === lvl3Name.toLowerCase());
            if (!lvl3Exists) {
              lvl2Node.children.push({
                id: generateId(),
                name: lvl3Name,
                completed: false,
                children: [],
    ...
    setGoals(prev =>
      prev.map(g => {
        if (g.id === goalId) {
          ...
          const completion = recalculateGoalCompletion(finalChildren);
  ...
  const updateHabitStatus = (id: string, date: string, status: HabitDayStatus) => {
    if (date < todayDate || lockedHabitDates.includes(date)) {
      return; // Immutability guard for auto-locked or locked dates
    }
    setHabits(prev =>
      prev.map(h => {
        if (h.id === id) {

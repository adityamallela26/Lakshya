...
export const HabitsTab: React.FC = () => {
  ...
  // Custom month/year browsing for monthly tracker (Starts at June 2026 based on todayDate)
  ...
  const isDayLocked = (dateStr: string) => {
    return dateStr < todayDate || lockedHabitDates.includes(dateStr);
  };
                  ...
                  <div key={h.id} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-3xs flex flex-col justify-between gap-4">
                    <div>
                      <div className="flex justify-between items-start gap-4">
                        ...
                        {deletingHabitId === h.id ? (
                          <div className="flex items-center gap-1.5 shrink-0 bg-rose-50 px-2 py-1 rounded border border-rose-100">
                          ...
                          <button
                            ...
                            className="text-slate-450 hover:text-red-500 cursor-pointer p-1 rounded-lg hover:bg-slate-50 transition"
                            title="Delete Habit"
                          >
                    ...
                    <div className="border-t border-slate-100 pt-3 flex items-center justify-between text-xs">
                      <div>
                        ...
                        <span className={`inline-block text-[11px] font-black mt-0.5 uppercase ${
                          currentStatus === 'Done'
                          ...
                          {currentStatus} {currentStatus === 'Holiday' ? '☕' : `(Symbol: ${getAccessibilitySymbol(currentStatus)})`}
                        </span>
                        {lockedHabitDates.includes(todayDate) && (
                          <span className="block text-[8px] text-amber-600 font-extrabold uppercase mt-0.5">🔒 LOCKED (Read-Only)</span>
                        )}
                        {!isHabitScheduled(h, todayDate) && (
                          <span className="block text-[8px] text-slate-400 font-bold uppercase mt-0.5">Not Scheduled Today</span>
                        )}
                      </div>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className={`px-3 py-1 text-[11px] font-bold rounded-lg transition-colors ${
                            lockedHabitDates.includes(todayDate) || !isHabitScheduled(h, todayDate)
                              ? 'cursor-not-allowed opacity-50 bg-slate-100 border border-slate-200 text-slate-405'
                              : currentStatus === 'Missed'
                              ? 'bg-red-600 text-white cursor-pointer hover:bg-red-750'
                              : 'bg-slate-50 border border-slate-200 text-slate-650 hover:bg-red-50 cursor-pointer'
                          }`}
                        >
                          ✗ Miss
                        </button>
                    ...
                    <div className="border-t border-slate-100 pt-3">
                      <span className="block text-[9px] font-extrabold uppercase text-slate-400 tracking-wider mb-2">Past 28 Days punchcard</span>

...
export const GoalsTab: React.FC = () => {
  ...
  const getNodeIdealCompletion = (node: GoalChild, goal: Goal, parentIdealDate?: string, parentDate?: string): number => {
    ...
    // If not and has children, rollup their pace.
    const hasCustomPace = node.idealDate || node.startDate;
    if (!hasCustomPace && node.children && node.children.length > 0) {
      const sum = node.children.reduce((acc, c) => acc + getNodeIdealCompletion(c, goal, finalIdealStr, finalDateStr), 0);
      return Math.round(sum / node.children.length);
    }
      ...
      {enteredGoalId === null ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                  {Array.from({ length: levelCount }).map((_, idx) => (
                    <div key={idx} className="bg-white p-2.5 rounded-lg border border-slate-200 shadow-2xs">
                      <span className="text-[10px] font-bold text-slate-400 block uppercase mb-1">Hierarchy Level {idx + 1}</span>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs text-slate-800 outline-none focus:border-indigo-600"
                      />
                    </div>
                  ))}
                </div>
        ...
        <div className="space-y-6 max-w-5xl mx-auto">
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm space-y-6">
                ...
                return (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {currentLevelNodes.map(item => {
                      ...
                      const nodeIdealPercent = getNodeIdealCompletion(item, activeGoal, parentIdealDate, parentDate);
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            {/* Card Name */}
                            <h4 className="text-sm font-extrabold text-slate-800 line-clamp-2 leading-snug group-hover:text-indigo-900 transition-colors">
                              {item.name}
                            </h4>
                      ...
                      <div
                        ...
                        className={`p-3.5 rounded-xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                          item.completed
                            ...
                            : 'bg-white border-slate-201 hover:border-slate-350 shadow-4xs'
                        }`}
                      >
                        ...
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <button
                            ...
                            className="text-indigo-600 hover:text-indigo-805 transition-colors cursor-pointer shrink-0"
                          ...
                          <div className="min-w-0 flex-1 flex flex-wrap items-center gap-y-1">
                            <span className="text-[9px] font-bold text-indigo-650 bg-indigo-50 px-1.5 py-0.5 rounded uppercase mr-2 tracking-wider select-none shrink-0 border border-indigo-150/50">
                              {activeLevelNames[currentDepth]}
                            </span>
                            <span className={`text-sm font-bold break-words mr-2.5 ${item.completed ? 'line-through text-slate-450 font-normal' : 'text-slate-800'}`}>
                              {item.name}
                            </span>
                            {isPastIdeal && (
                              <span className="text-[9.5px] font-mono font-black text-amber-700 bg-amber-100/70 border border-amber-250 rounded-md px-2 py-0.5 flex items-center gap-1 animate-pulse" title="Behind Ideal Timeline">
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 shrink-0 pl-8 sm:pl-0">
                          ...
                          <div className="flex flex-wrap gap-1.5 text-[9.5px] font-mono font-bold">
                            {item.startDate && (
                              <span className="text-slate-500 bg-slate-50 border border-slate-150 rounded px-1.5 py-0.5">
                                🏁 Onset: {item.startDate}
                              </span>
                            )}
                            {item.idealDate && (
                              <span className="text-emerald-700 bg-emerald-50/50 border border-emerald-150/55 rounded px-1.5 py-0.5">
                                ⚡ Pace: {item.idealDate}
                              </span>
                            )}
                            {item.date && (
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1.5 px-2.5 py-1 border rounded-lg text-xs font-bold transition-all cursor-pointer ${
                                item.classCompleted
                                  ...
                                  : 'bg-white text-slate-455 border-slate-200 hover:bg-slate-105'
                              }`}
                            >
                          ...
                          <div className="flex items-center gap-1 border-l border-slate-150 pl-2">
                            <button
                              type="button"
                              onClick={() => handleEditChildNode(activeGoal.id, item)}
                              className="p-1 text-slate-455 hover:text-indigo-650 hover:bg-slate-100 rounded transition"
                              title="Configure pacing parameters"
                            >
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            ...
                            <button
                              type="button"
                              onClick={() => moveChildGoal(activeGoal.id, item.id, 'up')}
                              className="p-1 text-slate-440 hover:text-indigo-650 hover:bg-slate-100 rounded"
                              title="Move Up"
                            >
                            ...
                            <button
                              type="button"
                              onClick={() => moveChildGoal(activeGoal.id, item.id, 'down')}
                              className="p-1 text-slate-440 hover:text-indigo-655 hover:bg-slate-100 rounded"
                              title="Move Down"
                            >
            ...
            {activeGoal && (
              <form onSubmit={handleAddInline} className="flex gap-2 pt-4 border-t border-slate-150/80 mt-4">
                <input
                  type="text"
                  value={inlineAddName}
                  onChange={(e) => setInlineAddName(e.target.value)}
                  placeholder={`➕ Fast Add ${activeLevelNames[currentDepth] || 'Objective Item'}...`}
                  className="flex-1 bg-slate-50 hover:bg-slate-50/50 focus:bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-850 outline-none focus:border-indigo-600 font-medium placeholder-slate-450 transition"
                />
                <button
                  type="submit"
                  disabled={!inlineAddName.trim()}
                  className="bg-indigo-650 hover:bg-indigo-700 active:scale-95 text-white font-extrabold px-4.5 py-2 rounded-xl text-xs cursor-pointer hover:shadow-3xs transition-all flex items-center gap-1 shadow-4xs shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {activeGoal?.classEnabled && isStudent && totalLeafCount > 0 && lagNodes.length > 0 && (
              <div className="bg-amber-50/50 border border-amber-150 rounded-xl p-4 space-y-2.5 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-2 max-h-[140px] overflow-y-auto pr-1">
                  {lagNodes.map(node => (
                    ...
                    >
                      <span className="text-slate-705 font-bold truncate max-w-[200px]" title={node.name}>
                        {node.name}
                      </span>
                      <button
                        type="button"
            ...
            <div className="bg-slate-50 border border-slate-200 p-4.5 rounded-xl space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
                <button
                  ...
                  className="bg-white hover:bg-slate-100 border border-slate-201 text-slate-705 px-3 py-2 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-4xs"
                >
                  <Download className="w-3.5 h-3.5 text-slate-500" />
                  <span>1-Node Outline (1 col)</span>
                </button>
                ...
                >
                  <Download className="w-3.5 h-3.5 text-slate-500" />
                  <span>2-Node Outline (2 cols)</span>
                </button>
                ...
                >
                  <Download className="w-3.5 h-3.5 text-slate-500" />
                  <span>3-Node Outline (3 cols)</span>
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-55 p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 max-w-md w-full p-6 space-y-4 text-slate-705 relative animate-scale-up">
            <button
              ...
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-605 p-1 rounded-lg cursor-pointer"
              title="Close Dialog"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div>
              <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-indigo-600 bg-indigo-50 p-1 rounded-md" />
            ...
            <div className="space-y-3.5">
              ...
              <div className="flex justify-end gap-2.5 pt-3.5 border-t border-slate-100 mt-2">
                <button
                  type="button"
                  onClick={() => setEditNodeModal(null)}
                  className="px-3.5 py-2 text-slate-500 hover:text-slate-705 font-bold text-xs cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={!editNodeModal.nodeName.trim()}
